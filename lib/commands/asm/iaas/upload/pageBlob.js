...
var uploadPageBlobFromBlobService =
  ...
  //console.log('blobService ' + JSON.stringify(blobService));
  //console.log('blobService.host ' + JSON.stringify(blobService.host));
  //verboseFunc(' to ' + blobService.host.primaryHost);
  verboseFunc(' to ' + blobService.host.primaryHost + blobResourceName);
  verboseFunc(' to ' +
  ...
  if (options.vhd) {
    ...
    if (vhdInfo.ddHeader) { // dynamic or difference
      dynVHD = true;
      blobService.createPageBlobFromLocalFile(container, blobName, fileName, function(error, result, response) {
        if(!error) {
          verboseFunc('file uploaded.');
          callback(null, false);
        }
      });
      return;
