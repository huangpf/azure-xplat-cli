...
function createStorageAccount(storageManagementClient, resourceGroupName, name, storageType, location, _) {
  var stoParams = {};
  stoParams.name = name;
  stoParams.location = location;
  stoParams.accountType = storageType;
  var account = storageManagementClient.storageAccounts.create(resourceGroupName, stoParams.name, stoParams, _);
  return account;
}

function createStorageResources(vmClient, storageManagementClient, resourceGroupName, name, storageType, hash, location, capacity, _) {
    ...
    for (var i = 0; i < capacity; i++) {
      ...
      if (i == 0) {
        ...
        if (createdAccount) {
          ...
          createStorageAccount(storageManagementClient, resourceGroupName, stoName, storageType, location, _);
        }
      }
      else {
        createStorageAccount(storageManagementClient, resourceGroupName, stoName, storageType, location, _);
...
exports.init = function (cli) {
  ...
  .execute(function (resourceGroupName, name, location, imageUrn, capacity, adminUsername, adminPassword, options, _) {
    ...
    var stoNames = createStorageResources(vmClient, storageManagementClient, resourceGroupName, name, options.storageType, hash, location, 5, _);
