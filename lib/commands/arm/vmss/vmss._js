...
exports.init = function (cli) {
  ...
  vmssQuickCreate.command('quick-create [resource-group-name] [name] [location] [image-urn] [admin-username] [admin-password]')
  ...
  .execute(function (resourceGroupName, name, location, imageUrn, adminUsername, adminPassword, options, _) {
    ...
    if (!options.sshPublickeyFile) {
      options.adminUsername = adminUsername = cli.interaction.promptIfNotGiven($('admin-username: '), adminUsername, _);
      options.adminPassword = adminPassword = cli.interaction.promptPasswordIfNotGiven($('admin-password: '), adminPassword, _);
    }
