...
var vmConstants = require('./vmConstants');
...
exports.init = function (cli) {
  ...
  vm.command('create [resource-group] [name] [location] [os-type]')
    ...
    .option('-q, --image-name <image-name>', $('the image name [image-name and image-urn parameters are mutually exclusive]'))
    .option('-Q, --image-urn <image-urn>', $('the image URN in the form publisherName:offer:skus:version [image-urn and image-name parameters are mutually exclusive]'))
    .option('-u, --admin-username <admin-username>', $('the user name' +
                  '\n      This parameter is valid for a VM created from an image (image-name, image-urn) and ignored when VM is based on an existing disk'))
    .option('-p, --admin-password <admin-password>', $('the password' +
                  '\n      This parameter is valid for a VM created from an image (image-name, image-ur) and ignored when VM is based on existing disk'))
    ...
    .option('-d, --os-disk-vhd <os-disk-vhd>', $('name or url of the OS disk Vhd' +
                   '\n     If this parameter is specified along with --image-name or --image-urn parameter then OS disk created from the image will be stored in this vhd' +
                   '\n     If this parameter is specified without --image-name or --image-urn parameter then this vhd must exists and will be used as OS Disk'))
  ...
  extension.command('get-image [location] [publisher] [typeName] [version]')
    ...
    docker.command('create [resource-group] [name] [location] [os-type]')
      ...
      .option('-E, --docker-extension-version <version>', $('Version of Docker Azure extension [0.6]'))
      ...
      .option('-Q, --image-urn <image-urn>', $('the image URN in the form publisherName:offer:skus:version [image-urn and image-name parameters are mutually exclusive]'))
      .option('-u, --admin-username <admin-username>', $('the user name' +
                   '\n      This parameter is valid for a VM created from an image (image-name, image-urn) and ignored when VM is based on disk (os-disk-*)'))
      .option('-p, --admin-password <admin-password>', $('the password' +
                   '\n      This parameter is valid for a VM created from an image (image-name, image-urn) and ignored when VM is based on disk (os-disk-*)'))
      .option('-M, --ssh-publickey-file <openssh-rsa-file|pem-file>', $('path to public key PEM file or SSH Public key file for SSH authentication' +
            '\n     This parameter is valid only when os-type is "Linux"'))
