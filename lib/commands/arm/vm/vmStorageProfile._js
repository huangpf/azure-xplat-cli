...
__.extend(VMStorageProfile.prototype, {
        generateStorageProfile: function(_) {
          ...
          if (storageProfile.osDiskDataDiskImgInfo.userImageVhd) {
            ...
            storageProfile.profile.osDisk.osType = this.params.osType;
            ...
            this.cli.output.info(util.format($('Using "%s" as the user image.'), storageProfile.osDiskDataDiskImgInfo.userImageVhd));
          }
        ...
        _parseOSDataDiskImageParams: function (params, _) {
            ...
            if (params.imageUrn){
              ...
              } else if (validation.isURL(params.imageUrn)) {
                osDiskDataDiskImageInfo.imageInfo = null;
                osDiskDataDiskImageInfo.userImageVhd = params.imageUrn;
              } else {
                throw new Error($('--image-urn must be in the form "publisherName:offer:skus:version", or a blob url to the user image VHD.'));
              }
        ...
        _parseOSDiskParams: function(params, createOption, _) {
          ...
          // Encryption
          if (this.params.diskEncryptionKeyVaultId || this.params.diskEncryptionKeySecretUrl) {
            osDiskInfo.profile.encryptionSettings = {
              diskEncryptionKey: {
                sourceVault: {
                  id: this.params.diskEncryptionKeyVaultId
                },
                secretUrl: this.params.diskEncryptionKeySecretUrl
              },
              keyEncryptionKey: null
            };
          }

          if (this.params.keyEncryptionKeyVaultId || this.params.keyEncryptionKeyUrl) {
            ...
            osDiskInfo.profile.encryptionSettings.keyEncryptionKey = {
              sourceVault: {
                id: this.params.keyEncryptionKeyVaultId
              },
              keyUrl: this.params.keyEncryptionKeyUrl
            };
          }
        ...
        _parseStorageCreateParams: function (params, _) {
          ...
          var createRequestProfile = {
            accountType: params.vmSize && utils.stringStartsWith(params.vmSize.toLowerCase(), 'standard_ds') ? 'Premium_LRS' : 'Standard_LRS',
            name: params.storageAccountName,
            location: params.location
          };
          
          if (createRequestProfile.accountType === 'Premium_LRS') {
            try {
        ...
        _generateNewStorageAccountNameWithPrefix: function (prefix, _) {
          if (utils.stringIsNullOrEmpty(prefix)) {
            prefix = 'cli' + (require('crypto').randomBytes(8, _).toString('hex'));
          }
          
          return this._normalizeString(prefix + (new Date()).getTime().toString());
        },
