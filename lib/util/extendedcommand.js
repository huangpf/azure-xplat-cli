...
_.extend(ExtendedCommand.prototype, {
  ...
  helpCategories: function(levels, depth) {
    ...
    for (var name in this.categories) {
      ...
      if (levels === -1 || levels > 0) {
        ...
        if (depth >= 1 && startsWith(cat.fullName(), 'vmss config ')) {
          var methods = [];
  ...
  helpVmssCategories: function(levels, depth) {
    for (var name in this.categories) {
      var cat = this.categories[name];
      if (depth === 0) {
        this.output.help('');
      ...
      if (levels === -1 || levels > 0) {
        ...
        var index = 0;
        var cmd = null;
        if (depth >= 1 && utilsCore.stringStartsWith(cat.fullName(), 'vmss config ')) {
          var commandNames = [];
        ...
        cat.helpVmssCategories(levels !== -1 ? --levels : -1, depth + 1);
      } else {
        this.output.help(' ', cat.fullName());
      }
    }
